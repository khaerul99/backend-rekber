  ...
  try {
    ...
    if (!isLongEnough || !hasUpperCase || !hasSpecialChar) {
      return res.status(400).json({
        message:
          "Password harus minimal 8 karakter, memiliki 1 huruf besar, dan 1 karakter unik (simbol).",
      });
    }
...
exports.verifyEmail = async (req, res) => {
  try {
    ...
    if (!user) {
      return res
        .status(400)
        .json({ message: "Token verifikasi tidak valid atau kedaluwarsa." });
    }
    ...
    res
      .status(200)
      .json({ message: "Email berhasil diverifikasi! Silahkan login." });
  } catch (error) {
    console.error(error);
  ...
  try {
    ...
    if (!user.isVerified) {
      return res
        ...
        .json({
          message:
            "Email Anda belum diverifikasi. Silahkan cek inbox email Anda.",
        });
    }
...
exports.resetPassword = async (req, res) => {
  try {
    ...
    if (!isLongEnough || !hasUpperCase || !hasSpecialChar) {
      return res.status(400).json({
        message:
          "Password baru harus minimal 8 karakter, memiliki 1 huruf besar, dan 1 simbol.",
      });
    }
